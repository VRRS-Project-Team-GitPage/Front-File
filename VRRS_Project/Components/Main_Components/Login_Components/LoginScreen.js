...
export default function LoginScreen({ navigation }) {
    ...
    const handleLogin = () => {
        const formBody = `id=${encodeURIComponent(idText)}&password=${encodeURIComponent(passwordText)}`;
        
        fetch('https://chaesigeodi.ddns.net/auth/login', {
            method: 'POST',
            body: formBody,
            headers: {
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                handleSave(); // 로그인 성공 시 유저 정보 저장
                checkLogin();
                AsyncStorage.setItem('user_id', data.data.id);
                navigation.navigate('Home');            
            } else {
                showToast('아이디와 비밀번호를 확인해주세요.'); // 에러 메시지 출력
            }
        })
        .catch((error) => {
            console.error('Error:', error);
            showToast('로그인 중 오류가 발생했습니다.'); // 네트워크 오류 메시지
        });
    };
